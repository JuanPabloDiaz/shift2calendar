...
const T = {
  es: {
    ...
    optEmailLabel: "Notificaciones por email",
  ...
  en: {
    ...
    optEmailLabel: "Email notifications",
...
async function _doCalendar(shifts, period) {
  ...
  const includeLocation = document.getElementById("locationToggle").checked;
  const location = includeLocation ? document.getElementById("locationSelect").value : "";
  ...
  const sendEmailNotifications = document.getElementById("emailToggle").checked;
  const locEl = document.getElementById("locationSelect");
  const locLabel = includeLocation ? locEl.options[locEl.selectedIndex].text : "";
  const reminderOverrides = getReminderOverrides();

  // Set sendUpdates parameter based on email toggle (none = no emails, all = send emails)
  const sendUpdatesParam = sendEmailNotifications ? "all" : "none";
  ...
  for (const s of merged) {
    ...
    const res = await fetch(
      `https://www.googleapis.com/calendar/v3/calendars/primary/events?sendUpdates=${sendUpdatesParam}`,
      {
        method: "POST",
        headers: {
...
function buildIcs(shifts) {
  ...
  const includeWeekly = document.getElementById("weeklyToggle").checked;
  const includeLunch = document.getElementById("lunchToggle").checked;
  const includeEmailAlarms = document.getElementById("emailToggle").checked;
  ...
  for (const s of merged) {
    ...
    // Email alarm (if enabled) - Note: Most calendar clients don't support this
    if (includeEmailAlarms) {
      lines.push(
        "BEGIN:VALARM",
        "ACTION:EMAIL",
        `DESCRIPTION:${t.descShift}`,
    ...
    if (extraMins > 0) {
      ...
      // Email alarm for extra reminder (if enabled)
      if (includeEmailAlarms) {
        lines.push(
          "BEGIN:VALARM",
          "ACTION:EMAIL",
    ...
    if (lunchReminderAt) {
      ...
      // Email alarm for lunch reminder (if enabled)
      if (includeEmailAlarms) {
        lunchLines.push(
          "BEGIN:VALARM",
          "ACTION:EMAIL",
...
document.getElementById("locationToggle").addEventListener("change", function () {
  ...
  locationSelect.style.display = this.checked ? "inline-block" : "none";
});
