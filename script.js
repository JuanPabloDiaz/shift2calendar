...
async function updateSummaryTab() {
  ...
  const values = [
    ...
    [""],
    ["💰 LATEST PAY PERIOD"],
    ["Pay Period", '=UNIQUE(FILTER(Schedule!H:H,Schedule!H:H<>"",Schedule!H:H<>"Pay period"),1,TRUE)'],
    ["Hours (latest period)", '=SUMIF(Schedule!H:H,B4,Schedule!E:E)', "hrs"],
    ["Estimated Pay (latest)", `=B5*${HOURLY_RATE}+SUMIF(Schedule!H:H,B4,Schedule!G:G)`, "$"],
    [""],
    [`📈 YEAR-TO-DATE TOTALS`],
    ["Total Hours Worked", `=SUMIF(Schedule!E:E,">0",Schedule!E:E)`, "hrs"],
    ["Regular Hours", `=B9-SUMIFS(Schedule!E:E,Schedule!B:B,"*Sunday*",Schedule!E:E,">0")`, "hrs"],
    ["Sunday Hours", `=SUMIFS(Schedule!E:E,Schedule!B:B,"*Sunday*",Schedule!E:E,">0")`, "hrs"],
    ["Regular Pay", `=B10*${HOURLY_RATE}`, "$"],
    ["Sunday Premium Pay", `=B11*${SUNDAY_RATE}`, "$"],
    ["Total Pay", "=B13+B14", "$"],
    ["Days Worked", '=COUNTA(UNIQUE(FILTER(Schedule!A:A,Schedule!A:A<>"",NOT(ISNUMBER(SEARCH("WEEK",Schedule!B:B))),NOT(ISNUMBER(SEARCH("BIWEEKLY",Schedule!B:B))))))', "days"],
    [""],
    ["🏥 BENEFITS & TIME OFF"],
    ["Sick Time Earned", `=B9*${SICK_TIME_ACCRUAL_RATE.toFixed(6)}`, "hrs"],
    ["Sick Time (h:mm)", '=TEXT(B19/24,"[h]:mm")'],
    ["Next Sick Hour At", `=CEILING(B9,30)`, "hrs worked"],
    ["Hours Until Next", `=B22-B9`, "hrs"],
    [""],
    ["🎯 CAREER MILESTONES"],
    ["Current Rate", `$${HOURLY_RATE}/hr`],
    ["Next Raise At", `=CEILING(B9/1000,1)*1000`, "hrs total"],
    ["Hours Until Raise", `=B27-B9`, "hrs"],
    ["Progress to Raise", `=B9/B27`, ""],
    [""],
    ["📊 QUICK STATS"],
    ["Total Weeks Worked", `=COUNTIF(Schedule!B:B,"*WEEK SUMMARY*")`, "weeks"],
    ["Avg Hours/Week", "=B9/B32", "hrs/wk"],
    ["Sunday Premium Total", "=B14", "$"],
    ["Avg Pay/Week", "=B15/B32", "$/wk"],
    [""],
    ["📅 MONTHLY BREAKDOWN"],
    ['=ARRAYFORMULA(QUERY({TEXT(Schedule!A:A,"yyyy-mm"),Schedule!E:E,Schedule!F:F},"select Col1,sum(Col2),sum(Col3),count(Col1) where Col1<>\'\' and Col2>0 and not Col1 contains \'WEEK\' and not Col1 contains \'BIWEEKLY\' group by Col1 order by Col1 desc label Col1 \'Month\', sum(Col2) \'Hours\', sum(Col3) \'Pay ($)\', count(Col1) \'Days\'",0))'],
  ];
